...
namespace DAL
{
...

    {
        ...
        public IEnumerable<Model.UserInfoNo>GetPage(Model.Page page)
        {
           using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(slelect row_number()over(order by username)as num, userinfo*from userinfo)";
                sql + = "selelect* from a  where num between(@PageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
        ...
        public int Delete(string username)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = " delete from userinfo wherr username = @userName";
